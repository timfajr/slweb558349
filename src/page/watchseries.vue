<template>
  ...
  page: function (){
    if ( this.page != "/watchseries/" + this.$route.params.token + "/"+ this.$route.params.roomid + "/" + this.$route.params.vid ){
  ...
  methods: {
    onStartup () {
        const api = domain + "/tvseries/gettitle?title=" + this.$route.params.title
        axios
            .get(api, {
                headers: {
